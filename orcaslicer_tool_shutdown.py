...
class ToolShutdownProcessor:
    ...
    def find_last_temperature_command_after_usage(self, tool_num, last_usage_line):
        """
        Find the last non-zero temperature command for a tool AFTER its last real usage.

        Searches backwards for generic M104 commands affecting this tool:

            M104 S<temp> T<tool_num>

        Any non-zero S value is treated as a target/idle temperature.
        Returns: index AFTER the matched line, or None.
        """
        # Pattern: M104 ... Sxx ... Tn (order of S/T does not matter)
        pattern = re.compile(
            rf'^\s*M104\b.*\bS(-?\d+(?:\.\d+)?)\b.*\bT{tool_num}\b',
            re.IGNORECASE
        )

        # Search from end of file backwards down to just after last usage
        for idx in range(len(self.lines) - 1, last_usage_line, -1):
            line = self.lines[idx]
            ...
            if abs(temp) < 1e-3:
                # Already an off command (S0) – nothing to override
                continue

            print(
                f"  ✓ Found: Last temperature command for T{tool_num} "
                f"at line {idx} (S{temp})"
            )
            return idx + 1  # Insert shutdown directly after this line

        print(f"  ℹ T{tool_num}: No temperature command found after last usage – active until end")
        return None

    def find_safe_insertion_point(self, last_usage_line, tool):
        """Find safe location to insert shutdown command for a tool."""
        insert_after_temp = self.find_last_temperature_command_after_usage(tool, last_usage_line)

        if insert_after_temp is not None:
            return insert_after_temp

        # No temperature command found – tool stays active until end
        return None
    ...
    def generate_shutdown_commands(self, tool_num):
        ...
        if self.config['add_comments']:
            ...
            commands.append("; Overrides OrcaSlicer idle temperature → 0°C\n")
            commands.append("; ========================================\n")
    ...
    def insert_shutdown_commands(self):
        ...
        # Sort tools by last usage line descending (latest first)
    ...
    def generate_report(self):
        ...
        report.append("; Script: orcaslicer_tool_shutdown.py v1.0.1\n")
    ...
    def process(self):
        ...
        print("=" * 60)
        print("OrcaSlicer Tool Shutdown v1.0.1")
        print("=" * 60)
...
def main():
    parser = argparse.ArgumentParser(
        description='OrcaSlicer Tool Shutdown v1.0.1 - Automatic energy-saving tool shutdown',
